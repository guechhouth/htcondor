...
-----------
  ...
  **htcondor snake submit** [**-\-jobdir <directory>**] [**snakefile**] [**-\- snakemake_args ...**]
      ...
      `snakemake-executor-plugin-htcondor <https://github.com/htcondor/snakemake-executor-plugin-htcondor>`_, 
      which in turn submits each Snakemake rule as its own HTCondor job.

      **snakefile** 
          Tell which Snakemake workflow file to run. If omitted, a file named ``Snakefile`` 
          in the current directory is used. **snakefile** can be specified 
          before or after **-\-jobdir**.
    
      **-\-jobdir <directory>** 
          Create a directory in the current working directory with the specified name.
          If omitted, a directory named **logs** will be created by default in the 
          current directory to store the management job logs.
    
      Anything after a **-\-** separator is passed through to Snakemake unmodified, so 
      additional Snakemake options can be supplied. 
  ...
  **htcondor snake halt** [**mgmt_id**]
          ...
          Gracefully halt all running jobs associated with the manager job and stop new jobs from being submitted.

          **mgmt_id**
              Tell which management job should be halt. Must be specified. 
